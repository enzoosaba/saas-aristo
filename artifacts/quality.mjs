...
const profile=mkdtempSync(join(tmpdir(),'aristo-quality-'));
...
try{
 ...
 for(const theme of ['dark','light']){for(const width of [390,1440]){await page.setViewportSize({width,height:1000});await page.goto(base);await page.locator('.app-shell').waitFor();await page.evaluate(t=>{localStorage.setItem('aristo-theme',t);document.documentElement.dataset.theme=t;},theme);for(const route of ['/','/rotina','/planos','/calendario','/perfil']){await page.goto(base+route);await page.locator('.app-shell').waitFor();result=await new AxeBuilder({page}).withTags(['wcag2a','wcag2aa','wcag21aa']).analyze();violations.push(...result.violations.map(v=>({route,theme,width,id:v.id,impact:v.impact,nodes:v.nodes.map(n=>n.target)})));}}}
